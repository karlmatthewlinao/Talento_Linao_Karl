...
export default function EntertainersPage() {
  ...
  const entertainers = [
    { name: 'Magician', image: '/entertainer.jpg', link: '/category/magician' },
    { name: 'Comedian', image: '/entertainer.jpg', link: '/category/comedian' },
    { name: 'Circus Performer', image: '/entertainer.jpg', link: '/category/circus-performer' },
    { name: 'Stand-Up Comedian', image: '/entertainer.jpg', link: '/category/standup-comedian' },
    { name: 'Impersonator', image: '/entertainer.jpg', link: '/category/impersonator' },
  ];
  ...
  return (
    ...
    >
      ...
      <div className="container mx-auto px-4 py-12 mt-16">
        <h2 className="text-3xl font-bold text-white mb-8">Entertainers</h2>
        <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-5 gap-8">
          {entertainers.map((entertainer, index) => (
            <Link key={index} href={entertainer.link} passHref>
              <div className="text-center cursor-pointer transform hover:scale-105 transition-transform">
                ...
                />
                <h3 className="text-lg font-semibold text-white">{entertainer.name}</h3>
              </div>
            </Link>
          ))}
        </div>
